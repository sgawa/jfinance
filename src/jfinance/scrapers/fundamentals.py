# このファイルは yfinance 1.5.2 の `scrapers/fundamentals.py` を tools/vendor_yfinance.py がコピーして作った。手で直さない。
# 元: https://github.com/ranaroussi/yfinance （Apache License 2.0, Copyright 2017-2019 Ran Aroussi）
# 中身: 財務諸表（時系列から表を組み立てる）
# 変えた点（Apache License 2.0 第 4 条 b）:
#   - 名前 yfinance → jfinance（import・ロガー名・repr）
#   - 差し替えた: 財務の開始日を jp._server から取る
#   - 差し替えた: freq に semiannual（半期報告書）を足す。日本は 2024 年に四半期報告書が廃止された
#   - 差し替えた: 同上
#   - 差し替えた: 既定で全年度を返す（yfinance は 2016-12-31 以降に固定）
#   - 差し替えた: jf.set_financials_basis() の連結区分・版をサーバへ送る（既定のときは何も足さない）
...
from jfinance import utils, const
...
class Financials:
    ...
    def _get_financials_time_series(self, timescale, keys: list) -> pd.DataFrame:
        ...
        period_qs = f"&period1={int(start_dt.timestamp())}&period2={int(end.timestamp())}" + _server.financials_query()  # jfinance: 連結区分・版
